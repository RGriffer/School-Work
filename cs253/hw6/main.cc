...
bool execute(vector<string> command, string outfile="") {
	...
	if (!outfile.empty()) {
	    close(1);
	    open(outfile.data(), O_WRONLY|O_CREAT|O_TRUNC, 0666);
	}
...
bool nestedParenCheck(string str) {
    ...
    for(unsigned long i = 0; i < str.size(); i++) {
        ...
        if(str[i-1] == '\\') {
         ...
         if(lineContent == "(" && isspace(str[i+1])) {
            ...
            if(line[1] != "(") {
                cerr << "ERROR: ./hw6: Second word is not an open parenthesis.\n";
                return false;
            }
                    
            else if(line[line.size()-1] != ")") {
                cerr << "ERROR: ./hw6: Last word is not a closing parenthesis.\n";
                return false;
            }
            /*else if(leftParen != 1 && rightParen != 1) {
                cerr << "ERROR: ./hw6: There are unescaped parenthesis other than the second work and last word.\n";
                return false;
            }*/
            else {
            ...
            if(line.size() - 1 > maxArg && maxArg != 0) { //Check for arguments that exceed given in -m
                cerr << "ERROR: ./hw6: amount of arguments for " << str << " exceeds maximum given in -m " << maxArg << ": " << str <<"\n";
                return false;
            }
            if(!execute(line)) {
                cerr << "ERROR: ./hw6: Can't run: " << str << "\n";
                return false;
            }
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc,argv, "vm:s:")) != -1) {
        switch(opt) {
            ...
            case 'm': //-m = max_args
                mCounter++;
                if(mCounter > 1) {
                    cerr << "ERROR: ./" << argv[0] << ": Called -s more than once. \n";
                    return 1;
                }
                num = atoi(optarg);
                if(num < 1 || num > 100) {
                    cerr << "ERROR: ./" << argv[0] << ": -m must contain a integer value between 1-100.\n";
                    return 1;
                }
                maxArgValue = num;
                num = 0;
            ...
            case 's': //-s = singleton characters specified
                ...
                break;
            default:
                cout << "ERROR: ./" << argv[0] << ": given option is not supported. please use -v/-m int/ -s \"string\"\n";
                return 1;
        }
    }
    ...
    if(argc > optind) {
        for (int i = optind; i < argc; i++) {
            infile1.open(argv[i]);
            if(!infile1.is_open()) {
                cerr << "./" << argv[0] << ": File \"" << argv[i] << "\" cannot be opened.\n";
                return 1;
            }
            else {
        ...
        cerr << "./" << argv[0] << ": error, cannot handle these arguments \n"; //If you see this you fucked up
    }
    return 0;
}
