...
###### Complete this function
    for i in range(N):
        for j in range(1, N):
             for k in range(j):
                currentTan = (image[i][k]-image[i][j])/(h*(j-k))
                if compare(math.tan(angle), currentTan): 
                    shade[i][j] = True
    return shade
...
###### Complete this function
    ...
    currMax = 0
    for i in range(N):
        currMax = image[i][0] + h*0*math.tan(angle)
        for j in range(1, N):
            LHS = image[i][j] + h*j*math.tan(angle)
            if compare(currMax, LHS):
                currMax = LHS
                continue
            if compare(LHS, currMax):
                shade[i][j] = True
